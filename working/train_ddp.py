...
train_crop_path = '/data/cheng/experiment/cropped_train_images/'
test_path = '/data/cheng/experiment/test_images/'
test_crop_path = '/data/cheng/experiment/cropped_test_images/'
...
CONFIG = {
	...
	'img_size': 384,
	'epochs': 20,
	'experiment_id': 'eff-b0-crop-images',
	'embedding_size': 512,
	'train_dataloader': {
		'batch_size': 64,
		'shuffle': True,
...
model_save_location = os.path.join(save_path, config.experiment_id, 'ckpt')
...
def Train(model, train_dataloader, epoch, optimizer, scheduler, scaler):
	model.train()
	loss_total = 0
	data_size = 0
	for batch_idx, (imgs, labels) in enumerate(train_dataloader):
		...
		if batch_idx % 10 == 0 and local_rank == 0:
			...
			print(
				f'Epoch [{epoch + 1}] {batch_idx}/{len(train_dataloader)} trainLoss: {reduced_loss.item()}当前batch精度：{thisbatch_acc}')
	if scheduler is not None:
		scheduler.step()
	# if batch_idx == 1:
	# 	break
...
for fold, (train_idx, val_idx) in enumerate(kf.split(train_metadata)):
	...
	for epoch in range(config.epochs):
		...
		model, train_avg_loss = Train(
			model, train_dataloader, epoch, optimizer, scheduler, scaler)
		# val
